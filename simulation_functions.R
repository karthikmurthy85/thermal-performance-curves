...
schoolfield_fun<- function(np2, mnval){
  ...
  # if(key == 1){
  #   Esh <- seq(0.001, 0.09, length.out=20)
  #   Edsh <- seq(0.001, 0.09, length.out=20)*5
  # }
  # 
  # if(key == 2){
  #   Esh <- seq(0.001, 0.09, length.out=20)*5
  #   Edsh <- seq(0.001, 0.09, length.out=20)
  # }
  Esh <- seq(0.001, 0.09, length.out=20)
  Edsh <- seq(0.001, 0.09, length.out=20)*5
  
  cc_optim <- NA;
  Eo <- Edo <- Tpko <- B0o <- Trefo <- NA
  for(ks in 1:20){
    
    E <-  mean(c(Esh[ks], 0.0001))
    Ed <- mean(c(Edsh[ks], 0.0001))
    
    
    optimal <- optim(c(B0, E, Ed, Tpk, Tref),fn=Wx,gr=NULL,method="L-BFGS-B",
                     lower=c(B0-0.01, 0.0001, 0.0001, (Tpk-1),(Tref -5)),
                     upper=c(B0+0.01, Esh[ks], Edsh[ks], (Tpk+1),(Tref +5)),
    ...
    cc_optim[ks] <- cc_score(np2$npp, Wt(B01, E1, Ed1, Tpk1, Tref1))
    B0o[ks] <- B01
    Eo[ks] <- E1
    Edo[ks] <- Ed1
    Tpko[ks] <- Tpk1
  ...
  id <- match(max(cc_optim, na.rm=T), cc_optim)
  B0 <- B0o[id]; E <- Eo[id]; Ed <- Edo[id]; Tpk <- Tpko[id]; Tref <- Trefo[id]
  
  cc_optim1 <- cc_score(np2$npp, Wt(B0, E, Ed, Tpk, Tref))
  
  optimal <- optim(c(B0, E, Ed, Tpk, Tref),fn=Wx,gr=NULL,method="L-BFGS-B",
                   lower=c(B0-0.1,E-0.01, Ed-0.01, (Tpk-1.5),(Tref -3)),
                   upper=c(B0+0.1, E+0.01, Ed+0.01,(Tpk+1.5),(Tref +3)),
                   control=list(maxit = 5000, pgtol = 1e-10,
                                ndeps = c(0.01, 1e-10, 1e-10, 0.1, 0.1),
                                lmm = 200))
  #optimal$par
  ...
  Tpk <- optimal$par[4]
  Tref <- optimal$par[5]
  cc_optim2 <- cc_score(np2$npp, Wt(B0, E, Ed, Tpk, Tref))
  ...
  for(ks in 1:20){
    ...
    optimal <- optim(c(B0, E, Ed, Tpk, Tref),fn=Wx,gr=NULL,method="L-BFGS-B",
                     lower=c(B0-0.01, 0.0001, 0.0001, (Tpk-0.25),(Tref -3)),
                     upper=c(B0+0.01, Esh[ks], Edsh[ks], (Tpk+0.25),(Tref +3)),
                     control=list(maxit = 5000, pgtol = 1e-10,
                                  ndeps = c(0.01, 1e-10, 1e-10, 0.1, 0.1),
                                  lmm = 200))
    print(optimal$par)
  ...
  optimal <- optim(c(B0, E, Ed, Tpk, Tref),fn=Wx,gr=NULL,method="L-BFGS-B",
                   lower=c(B0-0.1,E-0.01, Ed-0.01, (Tpk-0.25),(Tref -3)),
                   upper=c(B0+0.1, E+0.01, Ed+0.01, (Tpk+0.25),(Tref +3)),
                   control=list(maxit = 5000, pgtol = 1e-10,
                                ndeps = c(0.01, 1e-10, 1e-10, 0.1, 0.1),
                                lmm = 200))
  #optimal$par
  ...
  temp = np2$tmp
  tst1 <- data.frame(per = Wt1(B0, E, Ed, Tpk, Tref, temp),
                     temp = temp)
  #plot(np2, ty='o')
  #points(tst1$per ~ tst1$temp, ty='o', col='red')
  
  temp1 <- seq(-20,50,by=0.01)
  tst <- data.frame(per = Wt1(B0, E, Ed, Tpk,Tref, temp1)+ mnval,
                    temp = temp1)
  #plot(tst$per ~ tst$temp, ty='l', col='red',
  #ylim = c(0, 1), xlim = range(np2$tmp))
  #points(np2$npp+ mnval ~ np2$tmp, ty='o')
  
  np090 <- subset(tst, per > 0.9*max(tst$per, na.rm=T))
  #points(np090$per ~ np090$temp, ty='l', col='blue')
  diff(range(np090$temp))
  
  np075 <- subset(tst, per > 0.75*max(tst$per, na.rm=T))
  #points(np075$per ~ np075$temp, ty='l', col='blue')
  #range(np075$temp)
